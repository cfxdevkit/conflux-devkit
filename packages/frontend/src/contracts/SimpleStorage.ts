 ...
 *
 * This contract demonstrates basic storage functionality on both
 * Conflux Core Space and eSpace networks.
 */
...
export interface ContractTemplate {
  name: string;
  description: string;
  abi: unknown[]; // Use unknown[] to avoid strict typing issues with ABIs
  bytecode: string;
  constructorArgs: {
    name: string;
    type: string;
...
export const SIMPLE_STORAGE_TEMPLATE: ContractTemplate = {
  name: 'SimpleStorage',
  description:
    'A basic storage contract that can store and retrieve a name and value. Includes increment/decrement functionality.',
  ...
  constructorArgs: [
    {
      name: 'name',
      type: 'string',
      description: 'Initial name for the storage contract',
  ...
  functions: [
    {
      name: 'getValue',
      type: 'read',
      description: 'Get the current stored value',
      args: [],
    },
    {
      name: 'getName',
      type: 'read',
      description: 'Get the current stored name',
      args: [],
    },
    {
      name: 'setValue',
      ...
      args: [
        {
          name: 'newName',
          type: 'string',
          description: 'The new name to store',
        },
      ],
    },
    {
      name: 'increment',
      type: 'write',
      description: 'Increment the stored value by 1',
      args: [],
    },
    {
      name: 'decrement',
      type: 'write',
      description: 'Decrement the stored value by 1 (prevents negative)',
      args: [],
    },
  ],
};
