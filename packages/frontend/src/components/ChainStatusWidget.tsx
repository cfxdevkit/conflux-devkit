...
export function ChainStatusWidget({ currentNetwork }: ChainStatusWidgetProps) {
  ...
  const [espaceChainInfo, setEspaceChainInfo] = useState<ChainInfo | null>(null);

  // Mock chain status check - in real implementation, this would use actual clients
    ...
    <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          {coreChainInfo && (
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-mono">#{coreChainInfo.blockNumber.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Gas Price:</span>
                <span className="font-mono">{parseFloat(coreChainInfo.gasPrice) / 1e9} Gwei</span>
              </div>
              {coreChainInfo.error && (
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={getStatusColor(espaceChainInfo?.connected || false)}>
          ...
          {espaceChainInfo && (
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600">Block:</span>
                <span className="font-mono">#{espaceChainInfo.blockNumber.toLocaleString()}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Gas Price:</span>
                <span className="font-mono">{parseFloat(espaceChainInfo.gasPrice) / 1e9} Gwei</span>
              </div>
      ...
      <div className="mt-4 p-3 bg-blue-50 rounded-lg">
        <div className="flex items-start">
          <svg className="w-5 h-5 text-blue-600 mt-0.5 mr-2" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
          </svg>
          <div className="text-sm text-blue-800">
            <p className="font-medium">Network Information</p>
            <p className="mt-1">
              Connected to {currentNetwork} network. Chain status is checked every 10 seconds.
              {currentNetwork === 'testnet' && ' Use testnet faucets to get test tokens.'}
            </p>
          </div>
        </div>
      </div>
