#!/usr/bin/env node

/**
 * Contract Deployment Example with DevKit API
 *
 * This example demonstrates the new ergonomic DevKit API for contract deployment:
 * 1. Start the development node with DevKit
 * 2. Deploy SimpleStorage contract to Core Space
 * 3. Deploy SimpleStorage contract to eSpace
 * 4. Interact with both contracts
 * 5. Show streamlined cross-chain development workflow
 */

import { DevKit } from '../src/index.js';

// Simple storage contract for deployment
const SIMPLE_STORAGE_ABI = [
  {
    inputs: [
      { internalType: 'string', name: 'name', type: 'string' },
      { internalType: 'uint256', name: 'initialValue', type: 'uint256' },
    ],
    stateMutability: 'nonpayable',
    type: 'constructor',
  },
  {
    inputs: [],
    name: 'getValue',
    outputs: [{ internalType: 'uint256', name: '', type: 'uint256' }],
    stateMutability: 'view',
    type: 'function',
  },
  {
    inputs: [{ internalType: 'uint256', name: 'newValue', type: 'uint256' }],
    name: 'setValue',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [],
    name: 'getName',
    outputs: [{ internalType: 'string', name: '', type: 'string' }],
    stateMutability: 'view',
    type: 'function',
  },
  {
    inputs: [{ internalType: 'string', name: 'newName', type: 'string' }],
    name: 'setName',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [],
    name: 'increment',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
  {
    inputs: [],
    name: 'decrement',
    outputs: [],
    stateMutability: 'nonpayable',
    type: 'function',
  },
];

const SIMPLE_STORAGE_BYTECODE =
  '';

async function main() {
  console.log('🚀 Starting Contract Deployment Example with DevKit API\n');

  // Create DevKit instance with custom configuration
  const devkit = new DevKit({
    chainId: 2029,
    evmChainId: 2030,
    jsonrpcHttpPort: 12537,
    jsonrpcHttpEthPort: 8545,
    jsonrpcWsPort: 12535,
    log: false,
  });

  try {
    // ===== STEP 1: Start Development Node =====
    console.log('📦 Starting development node...');
    await devkit.start();
    console.log('✅ Node started successfully!\n');

    // Check mining status (DevKit starts mining automatically)
    const miningStatus = await devkit.getMiningStatus();
    console.log(
      `⛏️  Mining status: ${miningStatus.isRunning ? 'Running' : 'Stopped'}\n`
    );

    // Wait for mining to generate some blocks
    await new Promise((resolve) => setTimeout(resolve, 3000));

    // ===== STEP 2: Get Account Information =====
    const account0 = devkit.account(0);
    const balances = await account0.getBalances();

    console.log('👤 Using deployment account:');
    console.log(`  Core: ${account0.address.core} (${balances.core} CFX)`);
    console.log(`  eSpace: ${account0.address.evm} (${balances.evm} CFX)\n`);

    // ===== STEP 3: Deploy to Core Space =====
    console.log('🔗 Deploying SimpleStorage to Core Space...');

    const coreContractResult = await devkit.deployContract({
      abi: SIMPLE_STORAGE_ABI,
      bytecode: SIMPLE_STORAGE_BYTECODE,
      args: ['CoreSimpleStorage', 10n],
      chain: 'core',
      account: 0,
    });

    console.log(
      `✅ Core Space contract deployed at: ${coreContractResult.core}\n`
    );

    // ===== STEP 4: Deploy to eSpace =====
    console.log('🔗 Deploying SimpleStorage to eSpace...');

    const evmContractResult = await devkit.deployContract({
      abi: SIMPLE_STORAGE_ABI,
      bytecode: SIMPLE_STORAGE_BYTECODE,
      args: ['eSpaceSimpleStorage', 20n],
      chain: 'evm',
      account: 0,
    });

    console.log(`✅ eSpace contract deployed at: ${evmContractResult.evm}\n`);

    // ===== STEP 5: Test Core Space Contract =====
    console.log('🧪 Testing Core Space contract...');

    // Set value on Core Space
    console.log('  Setting value to 42...');
    const coreWriteResult = await devkit.writeContract({
      address: coreContractResult.core || '',
      abi: SIMPLE_STORAGE_ABI,
      functionName: 'setValue',
      args: [42n],
      chain: 'core',
      account: 0,
    });
    console.log(`  Transaction hash: ${coreWriteResult}`);

    // Read value from Core Space
    const coreValue = await devkit.readContract({
      address: coreContractResult.core || '',
      abi: SIMPLE_STORAGE_ABI,
      functionName: 'getValue',
      chain: 'core',
    });
    console.log(`  ✅ Core Space stored value: ${coreValue}\n`);

    // ===== STEP 6: Test eSpace Contract =====
    console.log('🧪 Testing eSpace contract...');

    // Set value on eSpace
    console.log('  Setting value to 84...');
    const evmWriteResult = await devkit.writeContract({
      address: evmContractResult.evm || '',
      abi: SIMPLE_STORAGE_ABI,
      functionName: 'setValue',
      args: [84n],
      chain: 'evm',
      account: 0,
    });
    console.log(`  Transaction hash: ${evmWriteResult}`);

    // Read value from eSpace
    const evmValue = await devkit.readContract({
      address: evmContractResult.evm || '',
      abi: SIMPLE_STORAGE_ABI,
      functionName: 'getValue',
      chain: 'evm',
    });
    console.log(`  ✅ eSpace stored value: ${evmValue}\n`);

    // ===== STEP 7: Summary =====
    console.log('📋 Deployment Summary:');
    console.log(
      `  Core Space: ${coreContractResult.core} (value: ${coreValue})`
    );
    console.log(`  eSpace: ${evmContractResult.evm} (value: ${evmValue})`);
    console.log('\n🎉 Contract deployment example completed successfully!');

    console.log('\n💡 DevKit Workflow:');
    console.log('  1. Create DevKit instance with configuration');
    console.log('  2. Start node with devkit.start()');
    console.log('  3. Deploy contracts using devkit.deployContract()');
    console.log(
      '  4. Interact with contracts using devkit.readContract() and devkit.writeContract()'
    );
    console.log('  5. Access accounts with devkit.account(index)');
    console.log('  6. Unified API works identically for both chains\n');
  } catch (error) {
    console.error('❌ Error in contract deployment:', error);
    if (error instanceof Error) {
      console.error('Error details:', error.message);
      console.error('Stack trace:', error.stack);
    }
    process.exit(1);
  } finally {
    // Cleanup
    console.log('🧹 Stopping development node...');
    await devkit.stop();
    console.log('✅ Node stopped cleanly');
  }
}

// Handle process termination gracefully
process.on('SIGINT', async () => {
  console.log('\n🛑 Received interrupt signal, shutting down...');
  process.exit(0);
});

process.on('SIGTERM', async () => {
  console.log('\n🛑 Received terminate signal, shutting down...');
  process.exit(0);
});

// Run the example
if (import.meta.url === `file://${process.argv[1]}`) {
  main().catch(console.error);
}

export default main;
