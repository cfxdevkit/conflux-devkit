...
export interface ClientManagerConfig {
  ...
  evm: ClientConfig;
  /** Server configuration (optional - for local development) */
  server?: ServerConfig;
...
export class ClientManager extends EventEmitter<ClientManagerEvents> {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (this.config.server) {
        this.serverManager = new ServerManager(this.config.server);
    ...
    return {
      ...
      server: this.serverManager
        ? {
            ...
            corePort: this.serverManager.getConfig().coreRpcPort || 12537,
            evmPort: this.serverManager.getConfig().evmRpcPort || 8545,
            wsPort: this.serverManager.getConfig().wsPort,
          }
        : undefined,
      networkSelector: {
  ...
  async startServer(): Promise<void> {
    ...
    // Network selector is automatically updated by ServerManager
    // Reinitialize clients to connect to local chains
    await this.initializeClients();

    this.emit('server:started', {
      coreChainId: 2029 as SupportedChainId,
      evmChainId: 2030 as SupportedChainId,
    });
  }
  ...
  async stopServer(): Promise<void> {
    ...
    // Reinitialize clients to reconnect to public chains
    await this.initializeClients();
