...
export class KeystoreService {
  ...
  constructor(keystorePath?: string) {
    this.keystorePath =
      keystorePath || join(homedir(), '.devkit.keystore.json');
  ...
  async initialize(): Promise<void> {
    ...
    if (this.keystore.length === 0) {
      const defaultMnemonic =
        process.env.HARDHAT_VAR_DEPLOYER_MNEMONIC || DEFAULT_MNEMONIC;
    ...
    if (!this.adminPrivateKey && this.keystore.length > 0) {
      try {
        ...
        if (this.encryptionEnabled && !this.encryptionPassword) {
          logger.warn(
            '⚠️  Cannot auto-set admin: keystore is encrypted and locked'
          );
        } else {
          throw error;
        }
  ...
  private async loadKeystore(): Promise<void> {
    try {
      if (existsSync(this.keystorePath)) {
        ...
        logger.info(
          `Loaded keystore with ${this.keystore.length} entries (encryption: ${this.encryptionEnabled ? 'enabled' : 'disabled'})`
        );
      }
    } catch (error) {
  ...
  async getDataDirInfo(): Promise<
    Array<{ index: number; label: string; dataDir: string; hash: string }>
  > {
    return Promise.all(
      this.keystore.map(async (entry, index) => ({
        index,
        label: entry.label,
  ...
  async setActiveMnemonic(index: number): Promise<void> {
    ...
    await this.saveKeystore();
    logger.info(
      `Set active mnemonic to index ${index}: ${this.keystore[index].label}`
    );
  }
  ...
  async deriveAccount(options: {
    ...
    if (network === 'espace') {
      ...
      const account = civePrivateKeyToAccount(privateKey as `0x${string}`, {
