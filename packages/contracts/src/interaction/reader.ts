...
export class ContractReader {
   ...
   */
  async isContract(address: string, chain: 'core' | 'evm'): Promise<boolean> {
    try {
   ...
   */
  private async readFromCore<T>(options: ReadOptions): Promise<T> {
    const coreClient = this.clientManager.getCoreClient();
   ...
   */
  private async readFromEvm<T>(options: ReadOptions): Promise<T> {
    const evmClient = this.clientManager.getEvmClient();
